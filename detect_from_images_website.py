...
from database import mysqlCon, mysqlObj
...
def run_inference(model, category_index, image_path):
    if os.path.isdir(image_path):
        ...
        for i_path in image_paths:
            ...
            # assign data query
            query1 = """INSERT INTO proceeded_images (user_id, image_path, detected_classes) 
                        VALUES (%d, '%s', '%s')""" % (int((user_id)), file_name, TextForDetectedClasses)
...
if __name__ == '__main__':
    ...
    image_path = "C:\\xamppP\\htdocs\\Plantify\\uploads"
    ...
    while True:
        ...
        if len(input_initial_count) > 0:
            ...
            run_inference(detection_model, category_index, image_path)
            # Will enter the loop before completion of run_inference
            while True:
                # Get the updated list of files in the folder
                input_files = os.listdir(input_folder_path)
                output_files = os.listdir(output_folder_path)

                # If the length of proceeded images' folder == length of itself before analysis + length of uploaded images:
...
# python .\detect_from_images_website.py
